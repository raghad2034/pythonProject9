...
class GUI:
    def __init__(self):
        ...
        Label(self.m, text='Number of Days (for sleepover):').pack()
...
#        self.pet_list = []
    ...
    def calculate_price(self):
        pet = PetInventory(self.e1.get(), self.e2.get(), self.pet_type_var.get(), self.gender_var.get(),self.service_var.get(), int(self.e3.get()) if self.e3.get() else None)
        pet.calculate_price()
    ...
    def run(self):
        ...
        fh=open("PetServicesInfoo.txt", "wt")
        fh.write(f"{PetInventory.display()}\n")
        fh.write((f"{self.calculate_price()},{self.name}"))
        fh.close()
